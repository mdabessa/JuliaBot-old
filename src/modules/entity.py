...
class Command:
    ...
    async def trycommand(cls, message, content, bot):
        ...
        if cmd["price"] > 0:
            points = db.getpoints(
                message.author.id, message.guild.id, bot.db_connection
            )
            if points < cmd["price"]:
                await message.channel.send(
                    f'{message.author.mention}, Voce não possui coins suficiente, custo do comando é de `{cmd["price"]}c`'
                )
                return
            else:
                await message.channel.send(
                    f'{message.author.mention} comprou {cmd["name"]} por `{cmd["price"]}c`'
                )
                db.subpoints(
                    message.author.id, message.guild.id, cmd["price"], bot.db_connection
                )
...
class Script:
    ...
    def __init__(self, name, func_name, guild_id, *, time_out=60, refresh=False):
        ...
        if len(func) == 0:
            raise Script.FuncError(
                f'Nenhuma função registrada com o nome "{func_name}".'
            )
        func = func[0]

        script = Script.fetch_script(name, by="refname")
        if len(script) >= func["limit_by_name"]:
            raise Script.ScriptIndiceLimit(
                f'Scripts com o nome "{name}", não podem ser mais criados.'
            )
    ...
    @staticmethod
    def new_function(
        function, name=None, tag="default", triggers=["reaction"], limit_by_name=1
    ):
        func = dict()
...
class Client(discord.Client):
    ...
    async def on_ready(self):
        ...
        await self.change_presence(
            activity=discord.Game(f"{len(self.guilds)} Servidores!")
        )
    ...
    async def on_message(self, message):
        ...
        if Timer.timer("point_time_" + str(message.guild.id), pointstime, recreate=1):
            for member in message.guild.members:
                if member.status == "offline" or (
                    member.bot == True
                    and member.id not in db.get_allowed_bots(self.db_connection)
                ):
                    continue
        ...
        if auto_events:
            if (
                Timer.timer(
                    "event_time_" + str(message.guild.id),
                    randint(1000, 10000),
                    recreate=1,
                )
                == True
                ...
                try:
                    ...
                    db.editserver(
                        message.guild.id, self.db_connection, "eventchannel", None
                    )
                    eventchannel = message.channel
        ...
        try:
            ...
            if cmdchannel == None:
                ...
                db.editserver(
                    message.guild.id, self.db_connection, "commandchannel", None
                )
                cmdchannel = None
            ...
            if r["reminderdate"] < datetime.datetime.now():
                try:
                    ...
                    try:
                        ...
                        print(
                            "Não foi possivel recuperar a mensagem e nem o autor do lembrete!"
                        )
                        db.delreminder(r["id"], self.db_connection)
                ...
                for ani in animes:
                    try:
                        embed = discord.Embed(
                            title=ani["anime"],
                            url=ani["link"],
                            description=f'Episódio {ani["episode"]}',
                            color=self.color,
                        )
