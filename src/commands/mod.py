...
async def c_event(message, commandpar, bot):
    if commandpar != None:
        ...
        try:
            scr = entity.Script(
                f"created_{commandpar}_{message.guild.id}",
                eve["name"],
                message.guild.id,
                time_out=30,
            )
            await message.channel.send(
                f'{message.author.mention} evento `{eve["name"]}` criado com sucesso!'
            )
            ...
            raise entity.CommandError(
                f"{message.author.mention}, Número `máximo` de eventos simultâneos atingido!, finalize os eventos que ja estão ocorrendo ou"
...
entity.Command(
    ...
    func=c_event,
    category=category,
    desc=f"Criar um evento.",
    aliases=["c_evento", "createevent", "criarevento"],
    args=[["evento", "*"]],
    perm=1,
)
...
entity.Command(
    ...
    func=clean_events,
    category=category,
    desc="Limpar todos os eventos.",
    aliases=["limpar_eventos", "le"],
    perm=1,
)
...
async def setcoins(message, commandpar, bot):
    if commandpar != None:
        try:
            ...
            raise entity.CommandError(
                "Só `numeros inteiros` podem ser definidos como coins"
            )
        ...
        try:
            if len(message.mentions) > 0:
                ...
                for user in message.mentions:
                    ...
                    db.setpoints(
                        user.id, message.guild.id, int(pointspar), bot.db_connection
                    )

                await message.channel.send(
                    f'Coins definido como `{pointspar}` para : {", ".join(names)}'
                )
            else:
                db.setpoints(
                    message.author.id,
                ...
                await message.channel.send(
                    f"{message.author.mention} Seus Coins foram definido para `{pointspar}`"
                )
        except:
...
entity.Command(
    ...
    func=setcoins,
    category=category,
    desc=f"Definir os seus pontos, ou os dos usuarios marcados.",
    aliases=["definircoins", "defcoins"],
    args=[["coins", "*"], ["pessoa", "º"]],
    perm=1,
)
    ...
    if commandpar != None:
        ...
        try:
            if len(message.mentions) > 0:
                ...
                await message.channel.send(
                    f'`{pointspar}` Coins adicionados para : {", ".join(names)}'
                )
            else:
                db.addpoints(
                    message.author.id,
                ...
                await message.channel.send(
                    f"{message.author.mention} Foram adicionados `{pointspar}` coins."
                )
        except:
            raise entity.CommandError("Não foi possivel realizar esta ação :worried:")
    else:
...
entity.Command(
    ...
    func=addcoins,
    category=category,
    desc=f"Adicionar pontos.",
    aliases=["adicionarcoins"],
    args=[["coins", "*"], ["pessoa", "º"]],
    perm=1,
)
    ...
    if commandpar != None:
        ...
        try:
            if len(message.mentions) > 0:
                ...
                await message.channel.send(
                    f'`{pointspar}` Coins foram removidos de : {", ".join(names)}'
                )
            else:
                db.subpoints(
                    message.author.id,
                ...
                await message.channel.send(
                    f"{message.author.mention} Foram removidos `{pointspar}` coins."
                )
        except:
            raise entity.CommandError("Não foi possivel realizar esta ação! :worried:")
    else:
...
entity.Command(
    ...
    func=subcoins,
    category=category,
    desc=f"Remover pontos.",
    aliases=["removercoins", "removecoins"],
    args=[["coins", "*"], ["pessoa", "º"]],
    perm=1,
)
...
async def shopadditem(message, commandpar, bot):
    ...
    await message.channel.send(
        f"Item: `{item_name}` foi adicionado a loja por `{price}` coins!"
    )
...
entity.Command(
    ...
    func=shopadditem,
    category=category,
    desc=f"Adicionar um item a loja.",
    aliases=["adicionaritem"],
    args=[["preço", "*"], ["item", "*"]],
    perm=1,
)
...
async def shopdelitem(message, commandpar, bot):
    ...
    if i != None:
        ...
        await message.channel.send(
            f'{message.author.mention} removeu o item `{i["itemid"]} - {i["name"]}` da loja!'
        )

    else:
        raise entity.CommandError(
            f"{message.author.mention} o item `{commandpar}` não existe."
        )
...
entity.Command(
    ...
    func=shopdelitem,
    category=category,
    desc=f"Deletar itens da loja.",
    aliases=["deleteitem", "deletaritem", "removeritem", "removeitem"],
    args=[["id do item", "*"]],
    perm=1,
)
...
async def pin(message, commandpar, bot):
    if commandpar == None:
        ...
        scr = entity.Script(
            f"pinned_message_{message.guild.id}",
            "pin",
            message.guild.id,
            time_out=86400,
        )
...
entity.Command(
    ...
    func=pin,
    category=category,
    desc=f"Fixar uma mensagem no servidor.",
    aliases=["pinar", "fix", "fixar"],
    args=[["Mensagem", "*"]],
    perm=1,
)
...
entity.Command(
    ...
    func=unpin,
    category=category,
    desc=f'Desfixar uma mensagem fixada com o comando "pin".',
    aliases=["despinar", "unfix", "desfixar"],
    args=[["Mensagem", "*"]],
    perm=1,
)
