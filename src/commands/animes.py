    ...
    if commandpar == None:
        ...
        if channel == None:
            await message.channel.send(
                f"O servidor `{message.guild}` não possui um canal para as notifições de animes."
            )
        else:
            await message.channel.send(
                f"Canal de notificações de animes: <#{channel.id}>\n"
        ...
        if commandpar == ".":
            ...
            await message.channel.send(
                f"As notificações de animes foram desabilitadas."
            )
            ...
            db.editserver(
                message.guild.id, bot.db_connection, "anime_channel", str(channel.id)
            )
            await message.channel.send(
                f"O canal de notificações de animes foi definido para `<#{channel.id}>`"
            )
...
entity.Command(
    ...
    func=anime_channel,
    category=category,
    desc=f"Define o canal de notificações de novos episódios de animes.",
    aliases=["animecanal", "ac", "canalanime"],
    args=[["canal", "*"]],
    perm=1,
)
...
async def add_anime(message, commandpar, bot):
    if commandpar != None:
        ...
        scr = entity.Script(
            f"add_anime_confirm_{message.guild.id}",
            "add_anime_confirm",
            message.guild.id,
            time_out=20,
        )
...
entity.Command(
    ...
    func=add_anime,
    category=category,
    desc=f"Marque um anime para você ser notificado quando lancar episódio novo.",
    aliases=["aa"],
    args=[["anime", "*"]],
)
...
async def del_anime(message, commandpar, bot):
    if commandpar != None:
        ...
        try:
            ...
            raise entity.CommandError("Não consegui achar nenhum anime com esse nome.")
        scr = entity.Script(
            f"del_anime_confirm_{message.guild.id}",
            "del_anime_confirm",
            message.guild.id,
            time_out=20,
        )
...
entity.Command(
    ...
    func=del_anime,
    category=category,
    desc=f"Remova um anime da sua lista de notificações.",
    aliases=["da"],
    args=[["anime", "*"]],
)
...
async def anime_info(message, commandpar, bot):
    if commandpar != None:
        ...
        emb = Embed(
            title=anime["title"], description=anime["synopsis"], color=bot.color
        )
...
entity.Command(
    ...
    func=anime_info,
    category=category,
    desc=f"Informações sobre um anime.",
    aliases=["anime", "ai", "a"],
    args=[["anime", "*"]],
)
...
async def character(message, commandpar, bot):
    if commandpar != None:
        ...
        try:
            ...
            print(e)
            raise entity.CommandError(
                "Não consegui achar nenhum personagem com esse nome."
            )
...
entity.Command(
    ...
    func=character,
    category=category,
    desc=f"Informações sobre um personagem.",
...
async def anime_list(message, commandpar, bot):
    if commandpar == None:
        animes = db.get_anime_notifier(
            str(message.author.id), bot.db_connection, column="userid"
        )
        ...
        scr = entity.Script(
            f"list_animes_{message.guild.id}",
            "list_animes",
            message.guild.id,
            time_out=300,
        )
        ...
        if len(message.mentions) != 0:
            ...
            animes = db.get_anime_notifier(
                str(user.id), bot.db_connection, column="userid"
            )
            ...
            raise entity.CommandError(
                "Você precisa mencionar alguem para ver a lista dessa pessoa, ou usar o comando sem nenhum parametro para ver sua propria lista!"
            )
...
entity.Command(
    ...
    func=anime_list,
    category=category,
    desc=f"Liste todos os animes da sua lista, ou da pessoa mensionada.",
    aliases=["al"],
)
