...
async def _help(message, commandpar, bot):
    if commandpar == None:
        ...
        emb = discord.Embed(
            ...
            description=f"{prefix}help `comando`",
            color=bot.color,
        )
        ...
        for c in categories:
            ...
            if (
                len(
                    entity.Command.getcommandsbycategory(
                        c[0], message.guild.id, bot.db_connection
                    )
                )
                == 0
            ):
...
entity.Command(
    ...
    func=_help,
    category=category,
    desc="Listar todos os comandos e suas descrições.",
    aliases=["ajuda", "h"],
    args=[["comando", ""]],
)
...
entity.Command(
    ...
    func=getprefix,
    category=category,
    desc=f"Retorna o prefixo do bot no servidor.",
    aliases=["prefixo"],
)
...
entity.Command(
    ...
    func=ping,
    category=category,
    desc=f"Retorna a latência do bot.",
...
async def remindme(message, commandpar, bot):
    ...
    await message.reply(
        f'Eu irei te notificar no dia `{future.strftime("%d/%m/%Y %H:%M")}`!'
    )
...
entity.Command(
    ...
    func=remindme,
    category=category,
    desc="O bot irá te notificar no dia desejado, relembrando sua mensagem!",
    aliases=["rm", "relembreme", "lembrete", "remind", "clock"],
    args=[["tempo", "*"]],
)
