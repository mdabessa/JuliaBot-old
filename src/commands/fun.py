...
async def _duel(message, commandpar, bot):
    ...
    if db.getpoints(message.author.id, message.guild.id, bot.db_connection) < points:
        raise entity.CommandError(
            f"{message.author.mention} Você não possui pontos suficientes!"
        )

    try:
        scr = entity.Script(
            f"duel_{message.guild.id}", "duel", message.guild.id, time_out=60
        )
...
entity.Command(
    ...
    func=_duel,
    category=category,
    desc=f"Duele contra alguem valendo coins!",
    aliases=["duelo", "x1"],
    args=[["coins", "*"], ["pessoa", "*"]],
)
