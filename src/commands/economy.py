...
async def coins(message, commandpar, bot):
    if len(message.mentions) >= 1 and len(message.mentions) <= 3:
        ...
        await message.channel.send(
            f"{message.author.mention}, você possui `{points}` coins."
        )
...
entity.Command(
    ...
    func=coins,
    category=category,
    desc="Verificar os pontos.",
    aliases=["moedas", "money"],
    args=[["pessoa", "º"]],
)
...
async def coinsrank(message, commandpar, bot):
    ...
    for num, i in enumerate(rank):
        ...
        emb.add_field(
            name=f"{medal}{user.name}", value=f":coin:{points}c", inline=False
        )
        ...
        if commandpar == "all":
            ...
            try:
                ...
                raise entity.CommandError(
                    "Não posso roletar nada que não seja um `numero inteiro` :pensive:"
                )

        scr = entity.Script(
            f"roulette_{message.guild.id}", "roulette", message.guild.id, time_out=30
        )
...
entity.Command(
    name="shop", func=shop, category=category, desc=f"Loja de itens.", aliases=["loja"]
)
...
async def iteminfo(message, commandpar, bot):
    ...
    if item != None:
        ...
        emb.set_footer(
            text=f'{db.getserver(message.guild.id, bot.db_connection)["prefix"]}buy {item["itemid"]}'
        )
        await message.channel.send(embed=emb)
    else:
        raise entity.CommandError(
...
entity.Command(
    ...
    func=iteminfo,
    category=category,
    desc="Veja as informações de um item da loja.",
    aliases=["ii"],
    args=[["id do item", "*"]],
)
...
async def buyitem(message, commandpar, bot):
    ...
    if i != None:
        ...
        await message.channel.send(
            f'{message.author.mention} comprou `{i["itemid"]} - {i["name"]}` por `{i["price"]}c`.'
        )
...
async def distance(message, commandpar, bot):
    ...
    if dif < 0:
        await message.channel.send(
            f"{message.author.mention}, {message.mentions[0].mention} possui `{abs(dif)}` coins a `mais` que você!"
        )

    elif dif == 0:
        await message.channel.send(
            f"{message.author.mention}, {message.mentions[0].mention} possui a `mesma` quantidade de coins que você!"
        )

    elif dif > 0:
        await message.channel.send(
            f"{message.author.mention}, {message.mentions[0].mention} possui `{dif}` coins a `menos` que você!"
        )
...
entity.Command(
    ...
    func=distance,
    category=category,
    desc=f"Calcula a diferença de coins entre você e outra pessoa.",
    aliases=["distancia", "dist", "distcoins", "dc"],
    args=[["pessoa", "*"]],
)
