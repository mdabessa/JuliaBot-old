...
entity.Command(
    ...
    func=setprefix,
    category=category,
    desc=f"Mude o prefixo de comandos do bot.",
    aliases=["definirprefixo", "defprefixo"],
    args=[["prefixo", "*"]],
    perm=1,
)
...
async def addcmd(message, commandpar, bot):
    ...
    if " " in commandpar[0] or ":" in commandpar[0]:
        raise entity.CommandError(
            'O nome do comando não pode conter nenhum espaço ou dois pontos ":".'
        )

    if (
        entity.Command.getcommand(message.guild.id, commandpar[0], bot.db_connection)
        != None
    ):
        raise entity.CommandError("Um comando com esse nome ja existe!")

    db.addcommand(
        message.guild.id, bot.db_connection, commandpar[0], commandpar[1], commandpar[2]
    )

    emb = discord.Embed(
        title="Novo Comando:", description=commandpar[0], color=bot.color
    )
...
entity.Command(
    ...
    func=addcmd,
    category=category,
    desc=f"Adicione um comando personalizado, utilize virgula para separar os parametros. [Ainda em teste!]",
    aliases=["addcmd", "addcomando", "adicionarcomando"],
    args=[["comando", "*"], ["mensagem", "*"], ["descrição", "*"]],
    perm=1,
)
...
async def delcmd(message, commandpar, bot):
    ...
    if cmd["overwritten"] == 0:
        await message.channel.send(
            f"O comando `{commandpar}` é um comando próprio da {bot.user.mention}, ele não pode ser deletado!"
        )
    else:
        db.delcommand(message.guild.id, bot.db_connection, commandpar)
        await message.channel.send(
            f"O comando `{commandpar}` foi deletado com sucesso!"
        )
...
entity.Command(
    ...
    func=delcmd,
    category=category,
    desc=f"Delete um comando personalizado!",
    aliases=["delcmd", "deletecommand", "deletarcomando", "delcomando"],
    args=[["comando", "*"]],
    perm=1,
)
...
async def commandchannel(message, commandpar, bot):
    if commandpar == None:
        ...
        if channel == None:
            await message.channel.send(
                f"O servidor `{message.guild}` permite comandos em qualquer canal!\n"
            ...
            await message.channel.send(
                f"Canal de comandos: <#{channel.id}>\n"
                + f"Para mudar o canal de comandos, utilize `{prefix}cmdchannel #canal` ou `{prefix}cmdchannel .` para poder utilizar comandos em todos os canais."
            )
    else:
        if commandpar == ".":
            ...
            await message.channel.send(
                f"Comandos agora podem ser utilizados em `qualquer` canal."
            )
            ...
            try:
                ...
                raise entity.CommandError(
                    f"Nenhum canal com esse nome, marque o canal com `#` para selecionar o canal desejado."
                )
            ...
            db.editserver(
                message.guild.id, bot.db_connection, "commandchannel", str(channel.id)
            )
            await message.channel.send(
                f"O canal de comandos foi definido para `<#{channel.id}>`"
            )
...
entity.Command(
    ...
    func=commandchannel,
    category=category,
    desc=f"Modifique o canal de comandos!",
    aliases=["commandchannel", "cmdcanal", "canaldecomandos"],
    args=[["canal", ""]],
    perm=1,
)
...
async def eventchannel(message, commandpar, bot):
    if commandpar == None:
        ...
        if channel == None:
            await message.channel.send(
                f"O servidor `{message.guild}` permite que apareca eventos em qualquer canal!\n"
        ...
        if commandpar == ".":
            ...
            await message.channel.send(
                f"Eventos agora podem aparecer em `qualquer` canal."
            )
            ...
            db.editserver(
                message.guild.id, bot.db_connection, "eventchannel", str(channel.id)
            )
            await message.channel.send(
                f"O canal de eventos foi definido para `<#{channel.id}>`"
            )
...
entity.Command(
    ...
    func=eventchannel,
    category=category,
    desc=f"Modifique o canal de eventos!",
    aliases=["canaldeevento", "eventocanal"],
    args=[["canal", ""]],
    perm=1,
)
...
entity.Command(
    ...
    func=auto_event,
    category=category,
    desc=f"Desative ou ative os eventos automaticos.",
    aliases=["evento", "events", "eventos"],
    args=[["booleano", "*"]],
    perm=1,
)
