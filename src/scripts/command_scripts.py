...
async def duel(cache, par, bot):
    if cache["status"] == "created":
        ...
        m = await msg.channel.send(
            f"{msg.author.mention} desafia {vs.mention} para um duelo valendo `{points}` coins!"
        )
        ...
        if emoji == "👍" and vs == react:
            if randint(0, 1) == 1:
                await m.channel.send(
                    f"{vs.mention} aceitou o duelo e venceu! `{points}` coins // {author.mention} perdeu `{points}` coins :sob:"
                )
                ...
                await m.channel.send(
                    f"{vs.mention} aceitou o duelo e perdeu! `{points}` coins // {author.mention} ganhou `{points}` coins :sunglasses:"
                )
...
async def roulette(cache, par, bot):
    if cache["status"] == "created":
        ...
        m = await channel.send(
            f"{author.mention}, Tem certeza que deseja roletar `{points}` coins?"
        )
        await m.add_reaction("👍")
        await m.add_reaction("👎")
            ...
            if points < p:
                if randint(0, 100) < roulette_chance:
                    ...
                    await message.channel.send(
                        f"{author.mention} Ganhou `{points}` coins! :money_mouth:"
                    )
                else:
                    db.subpoints(author.id, message.guild.id, points, bot.db_connection)
                    await message.channel.send(
                        f"{author.mention} Perdeu `{points}` coins! :sob:"
                    )
                ...
                if randint(0, 100) < roulette_chance:
                    db.addpoints(author.id, message.guild.id, points, bot.db_connection)
                    await message.channel.send(
                        f"{author.mention} roletou tudo e ganhou `{points}` coins, dobrando sua fortuna! :sunglasses:"
                    )
                else:
                    db.subpoints(author.id, message.guild.id, points, bot.db_connection)
                    await message.channel.send(
                        f"{author.mention} roletou tudo e perdeu `{points}` coins, zerando seus pontos! :rofl: :rofl: :rofl:"
                    )

            else:
                await message.channel.send(
                    f"{author.mention}, Voce não possui pontos suficiente!"
                )

            cache["status"] = 0
        elif emoji == "👎" and author == react:
            await message.channel.send(
                f"{react.mention} cancelou a roleta! :no_entry_sign:"
            )
            cache["status"] = 0
...
async def add_anime_confirm(cache, par, bot):
    if cache["status"] == "created":
        ...
        emb = Embed(
            title="Confirmar anime para adicionar:", description=desc, color=bot.color
        )
        ...
        if emoji == "👍" and author == react:
            if (
                db.verify_anime_notifier(author.id, anime["mal_id"], bot.db_connection)
                == None
            ):
...
async def del_anime_confirm(cache, par, bot):
    if cache["status"] == "created":
        ...
        emb = Embed(
            title="Confirmar anime para remover:", description=desc, color=bot.color
        )
        emb.set_thumbnail(url=anime["image_url"])
    ...
    if cache["status"] == "created":
        ...
        for i, _anime in enumerate(_animes):
            ...
            if len(animes) > 0:
                ...
                if i != len(_animes) - 1:
                    emb.set_footer(
                        text=f"{index+1}/{len(animes)} animes  -  Pesquisando: {len(animes)}/{len(_animes)} animes."
                    )
                    sleep(4)
                else:
        ...
        if emoji == "❌" and author == emoji_author:
            if index < len(animes):
                ...
                await m.channel.send(
                    f'`{anime["title"]}` removido da sua lista com sucesso!'
                )
