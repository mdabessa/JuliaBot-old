...
async def gift(cache, par, bot):
    if cache["status"] == "created":
        ...
        await message.channel.send(
            f"{user.mention} ganhou `{points}` coins, :partying_face:"
        )
        cache["status"] = 0
...
async def cards(cache, par, bot):
    if cache["status"] == "created":
        ...
        m = await channel.send(
            f"Escolha uma cartinha `2 boas`, `1 ruim` :flower_playing_cards:"
        )
        await m.add_reaction("💴")
        await m.add_reaction("💶")
        await m.add_reaction("💷")
        ...
        if choice != 0 and (choice not in cache["choices"]):
            ...
            if pts > 0:
                await message.channel.send(
                    f"{user.mention} pegou uma das cartas `boas` e ganhou `{pts}` coins! :moneybag:"
                )
                db.addpoints(user.id, message.guild.id, pts, bot.db_connection)
            else:
                await message.channel.send(
                    f"{user.mention} escolheu a pior `carta` e perdeu `{-pts}` coins! :money_with_wings:"
                )
...
async def quiz(cache, par, bot):
    if cache["status"] == "created":
        ...
        if operator == "Soma":
            m = await channel.send(
                f"O quiz chegou!, Qual resultado da soma entre [{num1}] e [{num2}] ? // ex: 5+5 -> 10"
            )
            res = num1 + num2
        if operator == "Multiplicação":
            m = await channel.send(
                f"O quiz chegou!, Qual resultado da multiplicação entre [{num1}] e [{num2}] ? // ex: 5x5 -> 25"
            )
            res = num1 * num2
        if operator == "Divisão":
            m = await channel.send(
                f"O quiz chegou!, Qual resultado da divisão entre [{num1}] e [{num2}] ? // ex: 1/16 -> 0.0625 -> 0.062"
            )
            res = num1 / num2
            res = utils.nround(res)
        if operator == "Subtração":
            m = await channel.send(
                f"O quiz chegou!, Qual resultado da subtração entre [{num1}] e [{num2}] ? // ex: 2-4 -> -2"
            )
            res = num1 - num2
